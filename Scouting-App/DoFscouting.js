...
function clearScoutingFields() {
	...
	document.getElementById( "AutoHub-input" ).value = "0";

	// Clear the Teleop fields
	document.getElementById( "TeleHub-input" ).value = "0";

	// Clear the No Show toggle
	document.getElementById( "NoShow-input" ).checked = false;

    // Clear the Relayed toggle
	document.getElementById( "Relayed-input" ).checked = false;

    // Clear the Herded toggle
	document.getElementById( "Herded-input" ).checked = false;

    // Clear the Died toggle
...
function resetFormForNextMatch() {
	...
	// Zap the scout name and then clear out the scouting fields
...
function updateMatchStart( event ) {
	...
	// TODO: Why is there STILL no check for the match level changing ( e.g. Semifinals or Final)?  If that changed
