...
# database(s).
#
# This is the first version of get_event_matches_2026.  It is based off
# the get_event_matches_2025_v2.py to start but will have lots commented
# out just so I can see the raw match data format.
#
# As of 2026 we renamed TBAAUTHKEY to be TBA_AUTH_KEY to be more Pythonic
# going forward.

import json  # TEMPORARY DEBUG OUTPUT ONLY
import logging
import os
import sys
...
###############################################################################
###############################################################################
def get_event_matches(api_client: ApiClient, event_code: str) -> Optional[List[Dict]]:
    '''
    ...
    Parameters:
        ...
        event_code (str): The event code to get the matches for
    '''
    logger: logging.Logger = get_logger()
    ...
    try:
        api_response = event_api.get_event_matches(event_code)

        status_msg: str = f"There are {len(api_response)} matches reported for {event_code}"
        ...
        if data:
            for doc in tqdm(data, desc=f"Saving to {db_collection.name} ({db_identifier})"):
                ...
                if filter_criteria["_id"] is not None:
                    db_collection.replace_one(filter_criteria, doc, upsert=True)
                else:
...
def main() -> None:
    ...
    with ApiClient(configuration) as api_client:
        ...
        event_code: str = (
            input("Enter the event code for the event to get data for (or 'quit' to exit): ")
            .strip()
        )
        ...
        logger.info(f"The event code for this session is {event_code}")
...
#             scheduleCollection2: Collection = db2[cfg.V5_COL_SCHEDULE]
        ...
        match_data: Optional[List[Dict]] = get_event_matches(api_client, event_code)
...
#             pprint.pprint(match_data, indent=2, compact=False)
...
#             saveDataToMongo(match_data, matchCollection, "Primary")

#             if matchCollection2 is not None:
#                 saveDataToMongo(match_data, matchCollection2, "Secondary")
