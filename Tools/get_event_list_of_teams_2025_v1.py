# This Python script will pull the list of teams registered for an event
# and display it on the screen.
#
# This is an updated version of get_event_list_of_teams_2024_v3.py.
# Changes include:
#
# 1: Updated to the 2025 V5 configuration
# 2: Switched to module wide logger
#
# Example output:
# (venv) D:\Workspace\TBA-python>python get_event_list_of_teams_2025_v1.py
# Enter event code (or 'quit' to exit): 2025caoc
# 47 teams were reported for 2025caoc:
# 8
# 840
# 867
# 973
...
def main() -> None:
    ...
    eventCode = input("Enter event code (or 'quit' to exit): ").strip()
    ...
    with ApiClient(configuration) as api_client:
        ...
        # Create an instance of the Event API class
        api_instance = EventApi(api_client)

        try:
            api_response = api_instance.get_event_teams(eventCode)

            if api_response is None:
                # No data found, display error message with colorama
                err_msg: str = f"Unable to get team data for {eventCode}"
                logger.error(err_msg)
                print(f"{Fore.RED}{err_msg}.")
                print("Check your event code or try again later!")
            else:
                # Sort the list by team_number in ascending order (key argument)
                sorted_teams = sorted(api_response, key=lambda team: team.team_number)

                status_msg: str = f"{len(api_response)} teams were reported for {eventCode}:"
                logger.info(status_msg)
                print(status_msg)

                for team in sorted_teams:
                    status_msg = f"{team.team_number}"
                    logger.info(status_msg)
                    print(status_msg)
        except ApiException as e:
            err_msg: str = f"APIException when calling EventApi.get_event_teams: {e}"
            logger.error(err_msg)
            print(f"{Fore.RED}{err_msg}")
