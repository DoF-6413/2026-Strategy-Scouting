# This is the v2 version of the random scouting data creator for 2026s
# game.
#
...
#
# Output modes:
#   Default: NDJSON - one JSON object per line (no array wrapper)
#   -L flag: JSON array format (same as v1)

import argparse
from random import randint, shuffle, uniform

# Parse command line arguments
parser = argparse.ArgumentParser(description="Generate random 2026 FRC scouting data.")
parser.add_argument(
    "-L",
    action="store_true",
    help="Output as a JSON array (legacy format). Default is NDJSON (one object per line).",
)
...
for matchNum in range(1, 41):
    ...
    for team in range(6):
        ...
        # Output the record — format depends on -L flag
        if args.L:
            print("{")
            print(matchResults)
            print("},")
...
# Finish the bounding JSON container (legacy array mode only)
if args.L:
    print("]")
