...
# database or two.
#
# It is intended to collect "ground truth" scouting data for use in checking
# the accuracy of scouter data during training.  As such the data gets saved
# into the V5_COL_TRAINING collection instead of the V5_COL_SCOUTING collection
#
# This is based on the scouting_2026_v1.py script.  The changes
# include:
#
# 1: Added client.admin.command("ping") check in get_database() to verify
#    connectivity before returning the database handle.
# 2: Updated the generic exception message in get_database() to match the
#    style used in get_match_breakdown_2026_v3.py.
# 3: Updated db-is-None error blocks to match the style used in
...
def main() -> None:
    ...
    scoutingCollection: Collection = db[cfg.V5_COL_TRAINING]
    ...
    if creds.SECONDARY_CONNECTION_STRING:
        ...
        scoutingCollection2: Collection = db2[cfg.V5_COL_TRAINING]
    ...
    while True:
        ...
        if isinstance(matchData, dict):
            ...
            scoutingCollection.replace_one({"_id": matchID}, matchData, upsert=True)

            if scoutingCollection2 is not None:
                scoutingCollection2.replace_one({"_id": matchID}, matchData, upsert=True)
